...
is importable rather than notebook-only -- used here as:
  - the cheap surrogate `writeback.ResidualWriter` fits against the posterior to translate
    KO calibration results back into per-glacier (refreeze_frac, insul_scale, z0) deltas, and
...
# f_ref: fraction of surface melt+rain percolating into the column. 0 = no latent heat.
REFREEZE_FRAC_BOUNDS = (0.0, 1.0)
# f_ins: multiplier on the Calonne (2011) snow/firn conductivity. The published relations
# span Sturm (1997) 0.59 to Yen (1981) 1.09 at Calonne = 1.0; widened for calibration.
INSUL_SCALE_BOUNDS = (0.3, 2.0)
Z0_BOUNDS = (5.0, 200.0)
...
def cp_model_single(depth, T_maat, dT_firn_band, insul_scale, z0, refreeze_frac, is_firn):
    """C&P analytical temperature (degC) at a single depth (m).

    T(z) = min(0, T_maat + ins * exp(-z / z0))

    refreeze_frac scales the insulation amplitude in ice/ablation bands only:
      firn bands: ins = insul_scale * dT_firn_band          (refreeze_frac has no effect)
      ice  bands: ins = refreeze_frac * ICE_FRAC * insul_scale * dT_firn_band

    Identifiability: refreeze_frac is only constrained by glaciers with ablation-zone
    (ice-band) observations; for firn-only glaciers it stays at its prior/default.
    """
    if is_firn:
        ins = insul_scale * dT_firn_band
    else:
        ins = refreeze_frac * ICE_FRAC * insul_scale * dT_firn_band
    return min(0.0, T_maat + ins * np.exp(-depth / z0))


def cp_model(depths, T_maat, dT_firn_band, insul_scale, z0, refreeze_frac, is_firn_arr):
    """Vectorised cp_model_single over an array of depths (and per-depth is_firn flags)."""
    return np.array([
        cp_model_single(d, T_maat, dT_firn_band, insul_scale, z0, refreeze_frac, f)
...
def clip_params(refreeze_frac, insul_scale, z0):
    """Clip (refreeze_frac, insul_scale, z0) to the physical bounds GloGEM re-clips to after
    applying any transfer-model baseline or residual correction."""
    pf = min(max(refreeze_frac, REFREEZE_FRAC_BOUNDS[0]), REFREEZE_FRAC_BOUNDS[1])
    ds = min(max(insul_scale, INSUL_SCALE_BOUNDS[0]), INSUL_SCALE_BOUNDS[1])
